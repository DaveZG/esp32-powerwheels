...
#define USART_MASTERSLAVE_RX_BYTES 7   // Receive byte count including start '/' and stop character '\n'

// Variables which will be written by slave frame
extern FlagStatus beepsBackwards;
extern float currentDCSlave;
extern float realSpeedSlave;

#endif
#ifdef SLAVE
#define USART_MASTERSLAVE_TX_BYTES 7   // Transmit byte count including start '/' and stop character '\n'
...
void CheckUSARTMasterSlaveInput(uint8_t USARTBuffer[])
{
	...
	//get Slave current DC and Real Speed
	realSpeedSlave = (float)((USARTBuffer[2] << 8) | USARTBuffer[3]);
#endif
#ifdef SLAVE
...
void SendMaster(FlagStatus upperLEDMaster, FlagStatus lowerLEDMaster, FlagStatus mosfetOutMaster, FlagStatus beepsBackwards)
{
	...
	realSpeed_Format = (uint16_t)(realSpeed * 100);
	
	// Send answer
	buffer[index++] = '/';
	buffer[index++] = sendByte;
	buffer[index++] = (realSpeed_Format >> 8) & 0xFF;
	buffer[index++] = realSpeed_Format & 0xFF;
