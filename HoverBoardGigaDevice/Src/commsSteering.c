...
float currentDCSlave = 5.8;
float realSpeedSlave = 4.2;
...
void SendSteerDevice(void)
{
	...
	uint16_t currentBatteryVoltage_Format;
	uint16_t realSpeedValue_Format;
	uint16_t realSpeedSlave_Format;
	
	currentBatteryVoltage_Format = (uint16_t)(batteryVoltage*100);
	realSpeedValue_Format = (uint16_t)(realSpeed*100);
	realSpeedSlave_Format = (uint16_t)(realSpeedSlave);
	
	// Ask for steer input
	buffer[index++] = '/';
	//send current battery voltage, motor currents, other info
	buffer[index++] = (currentBatteryVoltage_Format >> 8) & 0xFF;; 
	buffer[index++] = currentBatteryVoltage_Format & 0xFF; 
	buffer[index++] = (realSpeedValue_Format >> 8) & 0xFF;
	buffer[index++] = realSpeedValue_Format & 0xFF;
	buffer[index++] = (realSpeedSlave_Format >> 8) & 0xFF;
	buffer[index++] = realSpeedSlave_Format & 0xFF;
	buffer[index++] = '\n';
...
void CheckUSARTSteerInput(uint8_t USARTBuffer[])
{
	...
	//Set Field weakening
	if(USARTBuffer[5] & 0x80){
		activateWeakening = SET;
	}
	else
	{
	...
	//Set/Reset Master, Slave, Upper and Lower LED's
